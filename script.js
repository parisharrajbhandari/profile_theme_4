// ================================================================
//  EDIT THIS OBJECT TO CONFIGURE YOUR DIGITAL BUSINESS PROFILE
// ================================================================
...
const businessProfile = {
    ...
    description: "We provide premium range of Curtains, carpets, vinyl parquets, laminate parquets, mattress, rugs etc to decorate your home and office.",
    ...
    whatsappMessage: "Hello, I found your business profile through your NFC card and would like to connect.",
...
function renderProfile(p) {
    ...
    // Split name for two-tier typography matching reference design
    ...
    setText('#brandCompany', p.company || p.name);
    setText('#brandPill', (p.company || p.name || 'NFC').split(' ').map(w => w[0]).join('').slice(0, 3).toUpperCase());
    ...
    setText('#heroHandle', p.handle || `@${firstName.toLowerCase()}`);
    ...
    if (logoImg) {
        if (p.logo) {
            ...
            logoImg.onerror = () => {
                logoImg.hidden = true;
                if (logoText) {
                    logoText.textContent = (p.company || 'htc').split(' ').map(w => w[0]).join('').slice(0, 3).toLowerCase();
    ...
    if ($('#phoneValueLink')) $('#phoneValueLink').href = `tel:${phoneDigits}`;
    if ($('#phoneCard')) $('#phoneCard').href = `tel:${phoneDigits}`;
    if ($('#ctaCallButton')) $('#ctaCallButton').href = `tel:${phoneDigits}`;
    if ($('#dockCall')) $('#dockCall').href = `tel:${phoneDigits}`;
